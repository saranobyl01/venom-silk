...
function SpecimenCard({ specimen }: { specimen: Specimen }) {
  return <Dialog.Root><article className="group relative bg-background p-4"><div className="relative aspect-[4/5] overflow-hidden"><img src={specimen.image} alt={specimen.common} width={1200} height={1504} loading="lazy" className="size-full object-cover transition duration-700 group-hover:scale-[1.025] group-hover:brightness-110" /><div className="card-overlay absolute inset-0" /><div className="absolute inset-x-0 bottom-0 p-5"><p className="eyebrow mb-2">{specimen.family}</p><h3 className="font-display text-3xl">{specimen.common}</h3><p className="mt-1 text-xs italic text-foreground/60">{specimen.scientific}</p><Dialog.Trigger asChild><Button variant="ghost" size="sm" className="mt-5 text-[0.62rem]">View study <ArrowRight size={13} /></Button></Dialog.Trigger></div></div></article><Dialog.Portal><Dialog.Overlay className="fixed inset-0 z-[70] bg-background/85 backdrop-blur-md data-[state=open]:animate-fade-in" /><Dialog.Content className="fixed left-1/2 top-1/2 z-[80] grid max-h-[88vh] w-[min(92vw,900px)] -translate-x-1/2 -translate-y-1/2 overflow-y-auto border border-border bg-card shadow-2xl md:grid-cols-2"><div className="min-h-72"><img src={specimen.image} alt={specimen.common} width={1200} height={1504} className="size-full object-cover" /></div><div className="relative flex flex-col justify-center p-7 sm:p-10"><Dialog.Close asChild><Button variant="icon" size="icon" className="absolute right-4 top-4" aria-label="Close details"><X size={17} /></Button></Dialog.Close><p className="eyebrow">{specimen.family} · {specimen.origin}</p><Dialog.Title className="mt-5 font-display text-5xl">{specimen.common}</Dialog.Title><Dialog.Description className="mt-2 italic text-primary">{specimen.scientific}</Dialog.Description><p className="mt-7 text-sm leading-7 text-muted-foreground">{specimen.description}</p><p className="mt-8 border-t border-border pt-5 text-xs leading-6 text-muted-foreground">This collection is presented for education and appreciation. Specialist guidance is essential for welfare, handling, and legal compliance.</p></div></Dialog.Content></Dialog.Portal></Dialog.Root>;
}
